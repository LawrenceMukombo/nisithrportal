...
function isEmployableDateOfBirth(value: unknown): boolean {
  ...
  const parsedStr = parseIsoDate(value);
  if (!parsedStr) return true;
  const parsed = new Date(`${parsedStr}T00:00:00Z`);
...
router.post("/employees", authMiddleware, requireRole("admin", "hr_officer"), async (req, res): Promise<void> => {
  try {
    ...
    const [employee] = await db.insert(employeesTable).values({
      ...
      startDate: parseIsoDate(startDate) || new Date().toISOString().split("T")[0],
      probationStartDate: parseIsoDate(probationStartDate),
      probationEndDate: parseIsoDate(probationEndDate),
      confirmationDate: parseIsoDate(confirmationDate),
    }).returning();
    ...
    logger.error({ err: error }, "Failed to create employee record");
...
async function handleUpdateEmployee(req: any, res: any): Promise<void> {
  try {
    ...
    if (dateOfBirth !== undefined && !isEmployableDateOfBirth(dateOfBirth)) {
      res.status(400).json({ error: "Date of birth must be a valid date for an employee aged 18 or older" });
      return;
    }

    const parsedPositionId = positionId !== undefined ? (positionId ? parseInt(String(positionId), 10) : null) : existing.positionId;
    const parsedDepartmentId = departmentId !== undefined ? (departmentId ? parseInt(String(departmentId), 10) : null) : existing.departmentId;
    const parsedSupervisorId = supervisorId !== undefined ? (supervisorId ? parseInt(String(supervisorId), 10) : null) : existing.supervisorId;

    const isPositionOrDeptChanged =
      (parsedPositionId !== existing.positionId) ||
      (parsedDepartmentId !== existing.departmentId) ||
    ...
    const [updated] = await db.update(employeesTable)
      .set({
        ...
        email: email !== undefined ? (email ? email.toLowerCase().trim() : null) : existing.email,
        phone: phone !== undefined ? phone : existing.phone,
        dateOfBirth: dateOfBirth !== undefined ? parseIsoDate(dateOfBirth) : existing.dateOfBirth,
        ...
        startDate: startDate !== undefined ? (parseIsoDate(startDate) || existing.startDate) : existing.startDate,
        probationStartDate: probationStartDate !== undefined ? parseIsoDate(probationStartDate) : existing.probationStartDate,
        probationEndDate: probationEndDate !== undefined ? parseIsoDate(probationEndDate) : existing.probationEndDate,
        confirmationDate: confirmationDate !== undefined ? parseIsoDate(confirmationDate) : existing.confirmationDate,
        separationDate: separationDate !== undefined ? parseIsoDate(separationDate) : existing.separationDate,
    ...
    logger.error({ err: error }, "Failed to update employee record");
    res.status(500).json({ error: error?.message || "Failed to update employee record" });
  }
}

// PATCH & PUT /api/employees/:id - Update employee master record
router.patch("/employees/:id", authMiddleware, requireRole("admin", "hr_officer"), handleUpdateEmployee);
router.put("/employees/:id", authMiddleware, requireRole("admin", "hr_officer"), handleUpdateEmployee);
