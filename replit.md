...
- **Database Schema**: Centralized `lib/db/src/schema/` defines tables for agencies, roles, users, departments, positions, jobs, candidates (with detailed sub-records like education, experience), applications, employees, contracts, AI scores, notifications, integration configurations/logs, password reset tokens, and saved_jobs (applicant job bookmarks).
- **Saved Jobs**: Applicants can bookmark jobs from the landing page. Bookmarks are stored in `saved_jobs` (applicant_id FK → candidates, job_id FK → jobs, unique constraint). API: `GET /api/saved-jobs`, `GET /api/saved-jobs/ids`, `POST /api/saved-jobs/:jobId`, `DELETE /api/saved-jobs/:jobId`. Saved jobs appear in the "My Applications" page under a "Saved Jobs" section. The hook is at `artifacts/hr-portal/src/hooks/use-saved-jobs.ts`.
- **Job Card UX**: Landing page job cards are fully clickable via an absolute link overlay. Each card has a share icon (copy link to clipboard) and a bookmark icon (save job; guests are redirected to login). HR management view shows a Close button for both "open" and "published" jobs.
