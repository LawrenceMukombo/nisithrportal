...
router.get("/dashboard/summary", authMiddleware, async (req, res): Promise<void> => {
  ...
  const employeeFilter = agencyId ? eq(employeesTable.agencyId, agencyId) : undefined;
  const jobFilter = agencyId ? eq(jobsTable.agencyId, agencyId) : undefined;
  ...
  const [departmentsCountResult] = agencyId
    ? await db.select({ count: count() }).from(departmentsTable).where(eq(departmentsTable.agencyId, agencyId))
...
router.get("/dashboard/workforce-gaps", authMiddleware, async (req, res): Promise<void> => {
  const agencyId = req.query.agency_id ? parseInt(req.query.agency_id as string, 10) : undefined;

  const departments = agencyId
...
router.get("/dashboard/contract-expiries", authMiddleware, async (req, res): Promise<void> => {
...
router.get("/dashboard/recruitment-pipeline", authMiddleware, async (_req, res): Promise<void> => {
  const statuses = ["applied", "shortlisted", "interview", "selected", "rejected"];
  const pipeline = await Promise.all(statuses.map(async (status) => {
    const [result] = await db.select({ count: count() }).from(applicationsTable)
